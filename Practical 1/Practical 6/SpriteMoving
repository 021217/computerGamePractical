...
enum spriteDirection { MOVEDOWN, MOVELEFT, MOVERIGHT, MOVEUP };
...
int numbersFrame = 0;
int frameCounter = 0;
int numberDirection = MOVEDOWN;
int numberSpeed = 5;
int defaultSpeed = numberSpeed;
int dashSpeed = 2;
// Variables to manage dashing
bool isDashing = false;
int dashDuration = 5; // Frames or time units for how long the dash lasts
int dashTimer = 0;
int dashSpeedIncrease = 20; // Amount of speed added during dash
...
void createSprite() {
    ...
    hr = D3DXCreateTextureFromFile(d3dDevice, "militia.png", &numbersTexture);
    numberPosition.x = 500;
    numberPosition.y = 500;
...
void render() {
    ...
    numbersRect.top = ((frameCounter % numberMaxFrame) / numberCol) * numberSpriteHeight + (numberSpriteHeight * numberDirection);
    numbersRect.bottom = numbersRect.top + numberSpriteHeight;
    ...
    textRect.right = 900;
    textRect.bottom = 125;
    string currentSpeed = "Current Speed: " + to_string(defaultSpeed) ;
    LPCSTR mySpeed = currentSpeed.c_str();
    font->DrawText(sprite, mySpeed, 25, &textRect, 0, D3DCOLOR_XRGB(255, 255, 255));
...
void update(int frames) {
    for (int i = 0; i < frames; i++) {
        if (diKeys[DIK_A]) {
            numberDirection = MOVELEFT;
            numberPosition.x = numberPosition.x - (numberSpeed );
            frameCounter++;
        }
        if (diKeys[DIK_D]) {
            numberDirection = MOVERIGHT;
            numberPosition.x = numberPosition.x + (numberSpeed );
            frameCounter++;
        }
        if (diKeys[DIK_W]) {
            numberDirection = MOVEUP;
            numberPosition.y = numberPosition.y - (numberSpeed );
            frameCounter++;
        }
        if (diKeys[DIK_S]) {
            numberDirection = MOVEDOWN;
            numberPosition.y = numberPosition.y + (numberSpeed );
            frameCounter++;
        }

        if (!diKeys[DIK_A] && !diKeys[DIK_D] && !diKeys[DIK_W]) {
            numberDirection = MOVEDOWN;
        }
        ...
        if (diKeys[DIK_SPACE] && !isDashing) {
            isDashing = true;
            dashTimer = dashDuration;
            numberSpeed += dashSpeedIncrease;
        }
        ...
        // If not dashing, gradually reset speed to default
        if (!diKeys[DIK_SPACE] && !isDashing && numberSpeed != defaultSpeed) {
            if (numberSpeed > defaultSpeed) {
                numberSpeed -= 1;
            }
            else if (numberSpeed < defaultSpeed) {
                numberSpeed += 1;
            }
        }
